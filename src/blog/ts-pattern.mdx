...
[TC39로의 제안 깃허브](https://github.com/tc39/proposal-pattern-matching?tab=readme-ov-file#userland-matching)에도 소개된 **`ts-pattern`은 타입스크립트 환경에서 패턴 매칭을 쉽게 표현할 수 있게 해주는 라이브러리다.**

더 안전하고 나은 조건을 작성할 수 있고, 복잡함을 간결하게 표현할 수 있으며 그에 따라 코드 가독성도 좋아지고 철저한 타입 검사를 통해 케이스 누락도 방지할 수 있다.
...
- **타입 추론을 통한 타입 안전성 확보**
- 철저한 타입 검사로 통해 **가능한 모든 경우를 `.exhaustive()`와 일치하도록 강제**
...
| 메서드                                                    | 설명                                                                                                           |
| --------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- |
| `exhaustive(): TOutput`                                   | - **입력 값과 일치하는 패턴이 없으면 타입 검사가 실패한다.**<br/>(철저한 타입 검사, Exhaustiveness Type Check) |
| `otherwise(handler: (value: TInput) => TOutput): TOutput` | - **일치하는 패턴이 없을 경우에 실행된다.**<br/>- `switch`의 `default`로 생각하면 편하다.                      |
| `run(): TOutput`                                          | - **일치하는 패턴이 컴파일 타임에 꼭 있어야 한다.**<br/>(아래 코드 참조)                                       |

```ts {1}
match('2024 years!').exhaustive();

/*
  위 코드는 NonExhaustiveError 명목으로 타입 검사가 실패한다.
  아래와 같이 수정하면 정상적으로 타입 검사를 수행할 수 있다.
...
- `isMatch(pattern, value): boolean`: 패턴과 값의 매치가 성공하면 결과를 `boolean` 값으로 알려준다.
- `isMatch(pattern): (value) => boolean` : 패턴만 넣고 함수를 실행하면 해당 패턴에 대한 타입 가드 함수가 반환된다.
...
또, **`P.select`**를 호출 시 인자로 입력한 데이터의 프로퍼티 이름을 넣으면 그 값을 바로 추출할 수 있다.
...
match(res)
  .with({ error: P.select(errorMessage) }, (message) => message)
  .otherwise(() => '');
```
...
**요구사항 구현에 부합하는 패턴과 구현 처리를 해주는 함수들을 선언해두었을 뿐이다.**

객체 형식의 패턴들이 많아 가독성 측면에서 조금 불편할 수 있다면, 해당 패턴들을 변수로 선언하여 조금 더 의미 있는 값으로 표현하고 재사용성도 할 수 있다.
주문 관련된 영역이 많아 비슷한 요구사항들을 처리하는 모듈이 많기 때문에 `{ status: P.union('신규', '취소') }` 패턴을 상수로 선언하고 그 상수를 참조하는 것으로도 코드의 재사용성과 가독성을 높일 수 있을 것이다.
